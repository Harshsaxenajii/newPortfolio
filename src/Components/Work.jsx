...
const PageSubComp = (props) => {
  return (
    <div className="flex justify-center items-center mt-[6rem] w-full ">
      <div className="flex flex-col ml-[25%] justify-start md:flex-row -mb-5 relative">
        <motion.div
          className="w-5/12"
          ...
          viewport={{ once: true }}
        >
          <img className="image" src={props.image} alt="Project Image" />
        </motion.div>
        <motion.div
          className="w-1/3 absolute  flex flex-col justify-center items-start left-[38%] h-[100%] gap-3"
          initial={{ x: -200, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.3, delay: 0.35 }}
          viewport={{ once: true }}
        >
          <div className="mx-20 text-2xl font-bold text-gray-500">
            {props.title}
          </div>
          <div className="bg-[#000000] w-full rounded-md  text-[#9ca8c6] shadow-black px-4 py-6 text-sm">
            <div>{props.desc}</div>
          </div>
  ...
  return (
    ...
    >
      <div className="flex flex-col justify-center items-center mb-10 ">
        <div className="heading text-4xl text-gray-100 ">My Projects </div>
